...
const protPct = computed(() =>
  Math.round(((nutritionTotals.value?.prot ?? 0) / macroSum.value) * 100),
)
const fatPct = computed(() =>
  Math.round(((nutritionTotals.value?.fat ?? 0) / macroSum.value) * 100),
)
const carbPct = computed(() =>
  Math.round(((nutritionTotals.value?.carb ?? 0) / macroSum.value) * 100),
)
