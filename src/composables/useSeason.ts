...
export function getSeasonStatus(
  ...
  if (isNow && !isNext) return 'ending' // encore dispo ce mois-ci, fini le mois prochain
  if (isNow) return 'in' // pleinement de saison
  if (!isNow && isNext) return 'starting' // pas encore, mais arrive le mois prochain
  return 'out'
}
